    ...
    <div className="min-h-screen bg-[#060a14] text-white flex flex-col">
      ...
      <div className="px-6 py-6 border-b border-[#2a3a55] max-w-2xl mx-auto w-full">
        <div className="flex justify-between items-center">
          ...
          <Link href="/dashboard/messages" className="text-[#4a9eff] hover:underline text-sm">← Alle gesprekken</Link>
        </div>
      </div>
            ...
            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${
                  isMine ? 'bg-[#4a9eff] text-white' : 'bg-[#101828] border border-[#2a3a55] text-gray-200'
                }`}
              >
                {msg.content}
              </div>
      ...
      <form onSubmit={handleSend} className="px-6 py-4 border-t border-[#2a3a55] max-w-2xl mx-auto w-full flex gap-2">
        <input
          type="text"
          value={content}
        ...
        />
        <button type="submit" className="bg-[#4a9eff] hover:bg-[#3a8eef] px-4 py-2 rounded-lg font-semibold transition">
          Versturen
        </button>
      </form>
    </div>
