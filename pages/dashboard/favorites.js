...
export default function Favorites() {
  ...
  const auctionFavorites = favorites
    ...
    .sort(
      (a, b) =>
        new Date(a.listing.auction.endsAt) - new Date(b.listing.auction.endsAt),
    );

  const otherFavorites = favorites.filter(
    (f) => f.listing.listingType !== "auction" || !f.listing.auction,
  );
          ...
          {favorites.length === 0 ? (
            ...
            <>
              ...
              {otherFavorites.length > 0 && (
                <div>
                  <h2 className="text-lg font-bold mb-4 text-[#4a9eff]">
                    Overige favorieten
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
