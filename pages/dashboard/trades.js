...
export default function Trades() {
  ...
  const statusLabel = { pending: 'In afwachting', accepted: 'Geaccepteerd', declined: 'Afgewezen' };
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {trades.map((trade) => (
                <div key={trade.id} className="bg-[#101828] border border-[#2a3a55] rounded-xl p-4">
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-semibold">{trade.listing?.title}</p>
                    <span className={`text-xs font-semibold ${statusColor[trade.status]}`}>
                      {statusLabel[trade.status]}
                    </span>
                  </div>
                  <p className="text-sm text-gray-400 mb-2">
                    {tab === 'received' ? `Van: ${trade.sender?.username}` : `Aan: ${trade.receiver?.username}`}
                  </p>
                  <p className="text-sm text-gray-300 mb-3">Aanbod: {trade.offerDescription}</p>
