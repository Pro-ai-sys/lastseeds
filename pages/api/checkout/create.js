...
export default async function handler(req, res) {
  ...
  if (listing.ownerId === user.userId) {
    return res
      .status(400)
      .json({ error: "Je kunt niet je eigen listing kopen" });
  }
  ...
  return res
    ...
    .json({
      error:
        "Betalen is nog niet beschikbaar tijdens de testfase. Dit komt binnenkort!",
    });

  const feePercentage = parseFloat(process.env.PLATFORM_FEE_PERCENTAGE || "7");
  const totalAmount = listing.price;
  const platformFee =
    Math.round(totalAmount * (feePercentage / 100) * 100) / 100;
