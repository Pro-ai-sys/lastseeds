        ...
        <div className="bg-[#101828] border border-[#2a3a55] rounded-2xl p-6 mb-10 flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {seller.username}&apos;s winkel
            </h1>
            {seller.city && (
              <p className="text-gray-400 text-sm">{seller.city}</p>
            )}
            {seller.bio && (
              <p className="text-gray-300 text-sm mt-1">{seller.bio}</p>
            )}
            <Link
              href={`/seller/${seller.id}`}
              className="text-[#4a9eff] hover:underline text-sm mt-2 inline-block"
            >
